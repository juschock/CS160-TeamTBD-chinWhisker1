...
def add_shaver(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                models.create_process(brand, model, type, qty, shave_image)
...
def edit_shaver(request):
    ...
    if('popup' in id):
        ...
        try:
            ...
            'qty':shaver.qty,'shave_image': shaver.shave_image})
        except ValidationError as err:
            for err in err.messages:
                form.add_error(None, err)
            action = 'ERROR'
        ...
        if form.is_valid():
            ...
            try:
                models.edit_process(id, brand, model, type, qty, shave_image)
